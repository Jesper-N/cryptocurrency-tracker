<script lang="ts">
  ...
  let best = $derived(
    [...coins]
      .filter(
        (coin) =>
          coin.percentChange30d !== null && num(coin.percentChange30d) > 0.05,
      )
...
<div class="mt-8 flex flex-col gap-y-12 pb-12">
  <div class="px-4 opacity-0" use:enter={0}>
    ...
    <div
      class="crosshair-corners crosshair-container grid grid-cols-1 md:grid-cols-3"
    >
      {#each cards as card, idx (card.title)}
        <div
          class={`flex flex-col ${idx !== cards.length - 1 ? "md:panel-border-r" : ""}`}
        >
          <div
            class="panel-border-b bg-muted/10 cursor-default p-3 select-none"
          >
            ...
            {#each card.rows as row, rank (row.id)}
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  <span class="text-muted-foreground w-4 text-[13px]"
                    >{rank + 1}</span
                  >
                  <a
                  ...
                  >
                    <span
                      class="val-technical font-medium group-hover:underline"
                      >{row.name}</span
                    >
                    <span
                      class="text-muted-foreground text-[13px] tracking-wider uppercase"
                    >
                      /{row.symbol}
                    </span>
                  </a>
    ...
    <div class="crosshair-corners crosshair-container">
      <Table.Root class="text-[15px]">
        <Table.Header class="bg-muted/10">
          <Table.Row
            class="border-border cursor-default select-none hover:bg-transparent"
          >
            ...
            >
            <Table.Head class="lbl-technical text-right"
              >Circulating Supply</Table.Head
            >
            <Table.Head class="lbl-technical w-[150px] text-center"
              >Last Hour</Table.Head
            >
          </Table.Row>
        </Table.Header>
        <Table.Body>
          {#each rows as row (row.id)}
            ...
            >
              ...
              <Table.Cell>
                ...
                <span
                  class="text-muted-foreground ml-1 text-[13px] tracking-wider uppercase"
                >
                  {row.symbol}
                </span>
              </Table.Cell>
              ...
              <Table.Cell
                class={`text-right text-[13px] tracking-wider ${row.tone1h}`}
              >
                {row.percent1h}
              </Table.Cell>
              <Table.Cell
                class={`text-right text-[13px] tracking-wider ${row.tone24h}`}
              >
                {row.percent24h}
              </Table.Cell>
              <Table.Cell
                class={`text-right text-[13px] tracking-wider ${row.tone7d}`}
              >
                {row.percent7d}
              </Table.Cell>
              <Table.Cell class="val-technical text-right"
                >{row.marketText}</Table.Cell
              >
              <Table.Cell
                class="val-technical text-muted-foreground text-right"
              >
                {row.volumeText}
              </Table.Cell>
              <Table.Cell class="val-technical text-right">
                {row.supplyText}
                <span class="text-muted-foreground ml-1 text-[13px]"
                  >{row.symbol}</span
                >
              </Table.Cell>
